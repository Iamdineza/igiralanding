...
const articles = [['Featured', 'Understanding Right of Way in Rwanda', 'blogFeatured'], ['Traffic Rules', 'The road rules every learner should know', 'blogTrafficRules'], ['Road Signs', 'Understanding warning road signs', 'blogRoadSigns'], ['Driving Guide', 'How to prepare for your driving theory test', 'blogDrivingGuide']] as const
function BlogPreview() { const [filter, setFilter] = useState('All'); const filters = ['All', 'Traffic Rules', 'Road Signs', 'Driving Guide']; const filtered = filter === 'All' ? articles : articles.filter(([, , key]) => articles.find((a) => a[2] === key)?.[0] === filter); return <section className="section blog-section" id="blog"><div className="container"><Reveal className="section-heading row-heading"><div><Pill>KNOWLEDGE HUB</Pill><h2>Learn beyond the <SouthingAccent>test.</SouthingAccent></h2></div><a className="text-link desktop-only" href="#blog">Explore All Articles <ArrowRight size={16} /></a></Reveal><div className="blog-toolbar"><div className="filter-list">{filters.map((item) => <button key={item} className={filter === item ? 'active' : ''} onClick={() => setFilter(item)}>{item}</button>)}</div><div className="search-box"><Search size={16} /><span>Search articles</span></div></div><div className="articles-grid">{filtered.map(([category, title, image]) => <Reveal className={`article-card ${category === 'Featured' ? 'featured' : ''}`} key={title}><ImagePlaceholder asset={image} /><div className="article-meta"><span>{category}</span><span>5 min read</span></div><h3>{title}</h3><a href="#blog" aria-label={`Read ${title}`}>Read article <ArrowRight size={15} /></a></Reveal>)}</div></div></section> }
