...
function LanguageSection() {
  ...
  useEffect(() => { const code = activeLanguage === 1 ? 'FR' : activeLanguage === 2 ? 'RW' : 'EN'; window.dispatchEvent(new CustomEvent('igira-language-change', { detail: code })) }, [activeLanguage])
  useEffect(() => {
    const timer = window.setInterval(() => setActiveLanguage((current) => (current + 1) % languages.length), 2200)
