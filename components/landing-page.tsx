...
function DeviceMockup({ frame, screenshot, className = '' }: { frame: FrameKey; screenshot: AssetKey; className?: string }) {
  ...
  if (shot.composed) return <div className={`device-mockup composed-mockup ${className}`} data-frame={frame} data-screenshot={screenshot}><img className="composed-device-image" src={shot.src} alt={shot.alt} loading="lazy" /></div>
...
function HighlightUnderline({ children }: { children: React.ReactNode }) { return <span className="highlight highlight-underline">{children}<svg viewBox="0 0 180 18" aria-hidden="true"><path d="M3 12 C 42 6, 86 17, 177 5" /></svg></span> }
function HighlightMarker({ children }: { children: React.ReactNode }) { return <span className="highlight highlight-marker">{children}</span> }
function HighlightCircle({ children }: { children: React.ReactNode }) { return <span className="highlight highlight-circle">{children}<svg viewBox="0 0 180 80" aria-hidden="true"><path d="M12 42 C 24 5, 157 3, 170 38 C 178 68, 33 85, 12 42" /></svg></span> }
...
function ScrollToTop() {
  ...
  return <AnimatePresence>{visible && <motion.button className="scroll-top" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }} aria-label="Scroll to top">↑<span>Top</span></motion.button>}</AnimatePresence>
}
...
function GoogleTranslateBridge() {
  useEffect(() => {
    ...
    const onLanguage = (event: Event) => {
      ...
      window.setTimeout(() => {
        const select = document.querySelector('.goog-te-combo') as GoogleTranslateSelect | null
        if (select) { select.value = code; select.dispatchEvent(new Event('change', { bubbles: true })) }
      }, 300)
    }
...
function LanguageSelector({ mobile = false }: { mobile?: boolean }) {
  ...
  return <div className={`language-select ${mobile ? 'language-select-mobile' : ''}`}>
    ...
    {open && <div className="language-menu" role="listbox" aria-label="Choose language">{languages.map((language) => <button key={language[0]} role="option" aria-selected={selected[0] === language[0]} onClick={() => { setSelected(language); setOpen(false); window.dispatchEvent(new CustomEvent('igira-language-change', { detail: language[0] })) }}><span aria-hidden="true">{language[1]}</span><span>{language[2]}</span><b>{language[0]}</b></button>)}</div>}
  </div>
}
const faqs = [
...
function HeroSection() { return <section className="hero" id="top"><div className="road-lines" aria-hidden="true" /><div className="container hero-grid"><Reveal className="hero-copy"><Pill>BUILT FOR RWANDA</Pill><h1>Prepare for the road with <SouthingAccent><HighlightUnderline>confidence.</HighlightUnderline></SouthingAccent></h1><p>Prepare for Rwanda&apos;s driving theory test with practice questions, mock tests and practical driving knowledge in English, French and Kinyarwanda.</p><div className="hero-actions"><a className="button button-yellow" href="https://www.igiraprovisoire.rw">Get Started <ArrowRight size={17} /></a><a className="text-link" href="#blog">Explore the Blog <ArrowRight size={16} /></a></div><div className="hero-trust"><Icon icon="solar:shield-check-bold-duotone" width="18" height="18" aria-hidden="true" /> A clearer way to prepare</div></Reveal><Reveal className="hero-visual"><div className="hero-desktop"><DeviceMockup frame="heroDesktopFrame" screenshot="heroDesktopScreenshot" /></div><div className="hero-mobile"><DeviceMockup frame="heroMobileFrame" screenshot="heroMobileScreenshot" /></div><div className="float-badge badge-one"><Icon icon="solar:check-circle-bold" width="17" height="17" aria-hidden="true" /><b>12/20</b><span>Passing Score</span></div><div className="float-badge badge-two"><Icon icon="solar:alarm-bold" width="17" height="17" aria-hidden="true" /><b>20 min</b><span>Mock Test</span></div><div className="float-badge badge-three"><Icon icon="solar:translation-bold" width="17" height="17" aria-hidden="true" /><b>3</b><span>Languages</span></div></Reveal></div></section> }

function HowItWorks() { const steps = [['01', 'Learn', 'Understand traffic rules, road signs and driving concepts.'], ['02', 'Practice', 'Test your knowledge using practice questions.'], ['03', 'Prepare', 'Take timed mock tests and prepare for the theory test.']]; return <section className="section how-section" id="how-it-works"><div className="container"><Reveal className="section-heading"><Icon icon="solar:map-arrow-right-bold-duotone" width="24" height="24" aria-hidden="true" /><Pill>THE METHOD</Pill><h2>Your preparation starts here.</h2><p>A simple, focused path from uncertainty to confidence on the road.</p></Reveal><div className="steps-grid">{steps.map(([num, title, copy]) => <Reveal className="step" key={num}><span className="step-number">{num}</span><h3>{title}</h3><p>{copy}</p></Reveal>)}</div></div></section> }

function ProductShowcase() { return <section className="section product-section section-texture" id="about"><div className="container"><Reveal className="section-heading"><Pill>THE PRODUCT</Pill><h2>Study smarter. Drive ready.</h2><p>Everything you need to turn theory into instincts, without the pressure.</p></Reveal><div className="feature-row"><div className="feature-copy"><span className="eyebrow">PRACTICE MODE</span><h3>Practice without the pressure.</h3><p>Build your understanding one question at a time. Learn from your answers and make progress you can feel.</p><a className="text-link" href="https://www.igiraprovisoire.rw">Get Started <ArrowRight size={16} /></a></div><DeviceMockup frame="practiceDesktopFrame" screenshot="practiceDesktopScreenshot" className="showcase-device" /></div><div className="feature-row reverse"><div className="feature-copy"><span className="eyebrow">MOBILE FIRST</span><h3>Prepare wherever you are.</h3><p>Turn a commute, a break or a quiet evening into meaningful preparation. Your progress goes where you go.</p><div className="language-row"><span className="feature-chip"><Icon icon="solar:translation-bold" width="16" height="16" aria-hidden="true" /> English</span><span className="feature-chip"><Icon icon="solar:translation-bold" width="16" height="16" aria-hidden="true" /> Français</span><span className="feature-chip"><Icon icon="solar:translation-bold" width="16" height="16" aria-hidden="true" /> Kinyarwanda</span></div></div><DeviceMockup frame="practiceMobileFrame" screenshot="practiceMobileScreenshot" className="showcase-device phone" /></div><div className="feature-row"><div className="feature-copy"><span className="eyebrow">MOCK TEST</span><h3>Test yourself before the real test.</h3><p>20 questions. 20 minutes. A 12/20 passing score. Practice the pace and build confidence before exam day.</p><div className="metric-line"><b>20</b><span>questions</span><b>20</b><span>minutes</span><b>12/20</b><span>to pass</span></div></div><DeviceMockup frame="mockTestDesktopFrame" screenshot="mockTestDesktopScreenshot" className="showcase-device" /></div></div></section> }

function TestimonialsSection() { const testimonials = [['Aline M.', 'Kigali', 'The practice questions made the road signs finally click. I felt much calmer walking into my theory test.'], ['Eric N.', 'Huye', 'I could study in short sessions after work and track what I still needed to improve.'], ['Sandrine U.', 'Musanze', 'Clear, practical and made for Rwanda. It feels much easier than studying from scattered notes.']] as const; const [active, setActive] = useState(0); const paused = useRef(false); useEffect(() => { const timer = window.setInterval(() => { if (!paused.current) setActive((current) => (current + 1) % testimonials.length) }, 5200); return () => window.clearInterval(timer) }, [testimonials.length]); return <section className="section testimonials-section" aria-labelledby="testimonials-title"><div className="container"><Reveal className="section-heading"><Icon icon="solar:chat-round-like-bold-duotone" width="24" height="24" aria-hidden="true" /><Pill>FROM THE ROAD</Pill><h2 id="testimonials-title">A little more confidence, from people like you.</h2><p>Real preparation feels better when you know you are not doing it alone.</p></Reveal><div className="testimonials-layout" onMouseEnter={() => { paused.current = true }} onMouseLeave={() => { paused.current = false }} onFocus={() => { paused.current = true }} onBlur={() => { paused.current = false }}><div className="testimonial-card" key={active}><span className="testimonial-quote">“</span><blockquote>{testimonials[active][2]}</blockquote><div className="testimonial-person"><strong>{testimonials[active][0]}</strong><span>{testimonials[active][1]}</span></div></div><aside className="google-review-card"><div className="google-review-top"><Icon icon="logos:google-icon" width="22" height="22" aria-hidden="true" /><strong>Google Reviews</strong></div><div className="review-score"><b>4.9</b><span aria-label="4.9 out of 5 stars">★★★★★</span></div><p>See what learners are saying about their preparation experience.</p><a className="text-link" href="https://www.google.com/search?q=Igira+Provisoire" target="_blank" rel="noreferrer">View on Google <ArrowRight size={15} /></a></aside></div><div className="testimonial-controls" aria-label="Choose testimonial">{testimonials.map(([name], index) => <button key={name} className={active === index ? 'active' : ''} onClick={() => setActive(index)} aria-label={`Show testimonial from ${name}`} aria-pressed={active === index}><span /></button>)}</div></div></section> }

function LanguageSection() {
  const languages = ['English', 'Français', 'Kinyarwanda']
  const [activeLanguage, setActiveLanguage] = useState(0)
  useEffect(() => { const onLanguage = (event: Event) => { const code = (event as CustomEvent<string>).detail; setActiveLanguage(code === 'FR' ? 1 : code === 'RW' ? 2 : 0) }; window.addEventListener('igira-language-change', onLanguage); return () => window.removeEventListener('igira-language-change', onLanguage) }, [])

  return <section className="language-section"><div className="container language-layout"><Reveal><Pill>MADE TO MEET YOU</Pill><h2>Learn in the language that <SouthingAccent>works for you.</SouthingAccent></h2><p>Clarity matters when you&apos;re learning the rules of the road. Choose the language that makes every concept click.</p></Reveal><Reveal className="language-display" aria-live="polite"><div className="language-highlight-track" aria-label="Supported languages">{languages.map((language, index) => <motion.button key={language} className={`language-highlight ${activeLanguage === index ? 'active-language' : ''}`} onClick={() => setActiveLanguage(index)} aria-pressed={activeLanguage === index}>{language}{activeLanguage === index && <motion.span className="language-highlight-bar" layoutId="language-highlight-bar" transition={{ type: 'spring', stiffness: 300, damping: 28 }} />}</motion.button>)}</div><p className="language-current">Now learning in <strong>{languages[activeLanguage]}</strong></p></Reveal></div></section> }
